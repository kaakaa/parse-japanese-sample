...
func main() {
	...
	for _, token := range tokens {
		...
		if token.Features()[0] == "名詞" && token.Features()[1] == "一般" {
			// features := strings.Join(token.Features(), ",")
			if v, ok := counter[token.Surface]; ok {
				counter[token.Surface] = v + 1
			} else {
...
func count(m map[string]int) {
	var es entries
	for k, v := range m {
		es = append(es, entry{key: k, count: v})
	}
	sort.Sort(sort.Reverse(es))
	for _, e := range es {
		log.Printf("%d: %s", e.count, e.key)
	}
}
...
次回のv5.3のリリースは2018/9/14(Fri)を予定しています。
